 ...
 */
function mapADEMEResultToDPE(raw: ADEMERawResult): DPE | null {
  ...
  const classeEnergetique = validClasses.includes(
    raw.classe_consommation_energie
  )
    ? (raw.classe_consommation_energie as ClasseEnergetique)
    : "N";
  const classeGES = validClasses.includes(raw.classe_estimation_ges)
    ? (raw.classe_estimation_ges as ClasseGES)
    : "N";
